...
found:
    ...
    boolean removeIf(Predicate<? super E> filter, int i, final int end) {
        ...
        if(i<end) {
            ...
            int w = beg;
            for(i = beg; i<end; i++) {
                //遍历deathRow数组，es记录赋值为0的位置，即不需要删除的位置
    ...
    //若传入数值为0-63则返回1，64-127则返回2，以此递增
    private static long[] nBits(int n) {
        return new long[((n - 1) >> 6) + 1];
    }
    //根据传入数值的大小即i和beg的差值，选择bits[]的不同元素，再按位将64位串相应的位置赋值为1
